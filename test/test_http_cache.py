...
class TimeSourceMixinAPI(http_cache.SyncHTTPMixin, abstract_source.AbstractMultiMessageSourceAPI):
    """Just queries the current time of day from a status API. Caches the result using the Mixin pattern"""
    ...
    def __init__(self, **kwargs):
        ...
        super(TimeSourceMixinAPI, self).__init__(**kwargs)
        if "DATA_CRAWLER_CONTACT" not in os.environ:
            raise KeyError("Please put your contact details in the environment variable DATA_CRAWLER_CONTACT.")
        self.session.headers["User-Agent"] = f"E3SchoolEMS_Test_Suite {os.environ['DATA_CRAWLER_CONTACT']}"

    def fetch_data_bundle(self) -> Generator[abstract_source.MessageData, None, None]:
        """Queries the status API and returns the result"""
...
def test_expiration_date_mixin():
    """Tests the caching mechanism using an expiration date and the mixin pattern"""

    api = TimeSourceMixinAPI(source_parameters={
        "cache": {
            "directory": ".cache-test",
            "expire": "2s"
        },
    ...
    call_0 = list(api.fetch_data_bundle())
    call_1 = list(api.fetch_data_bundle())
    time.sleep(3.0)
    call_2 = list(api.fetch_data_bundle())
    ...
    assert call_0[0]["header date"] == call_1[0]["header date"]
    assert call_0[0]["header date"] < call_2[0]["header date"]

    assert call_0[0]["last update"] == call_1[0]["last update"]
    assert call_0[0]["header expires"] == call_1[0]["header expires"]


def test_expiration_mixin_default():
    """Tests the default expiration behaviour according to the external website using the mixin pattern"""
